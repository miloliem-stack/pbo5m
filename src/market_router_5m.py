...
SLUG_PREFIX = os.getenv("BTC_5M_SLUG_PREFIX", "btc-updown-5m-").strip().lower()
...
ROUTER_UPCOMING_TOLERANCE_SECONDS = max(300, int(os.getenv("BTC_5M_ROUTER_UPCOMING_TOLERANCE_SECONDS", "1800")))
ROUTER_SAMPLE_LIMIT = max(1, int(os.getenv("BTC_5M_ROUTER_SAMPLE_LIMIT", "8")))
...
def _looks_like_btc_5m(candidate: dict[str, Any]) -> bool:
    ...
    if slug.startswith(SLUG_PREFIX):
        return True
    text = f"{slug} {title}"
    return any(word in text for word in BTC_KEYWORDS) and any(word in text for word in FIVE_MINUTE_WORDS)
...
def _filter_candidates(candidates: list[dict[str, Any]], *, now=None) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for candidate in candidates:
        ...
        if start is None or end is None or end <= start:
            ...
            continue
        if not _looks_like_btc_5m(candidate):
            diagnostics["exclusion_counts"]["not_btc_5m_family"] += 1
            continue
        diagnostics["counts"]["btc_5m_family_match"] += 1
